...
namespace BRONUF_Main.Main.Projects.GenerationsDoc
{
    public partial class Word_Gen
    {
        public bool GenAndSendListSveds(List<int> CurrentList, List<ProjectHelper> projectHelpers, int CountSved, string NameUser, TelegramBotClient _client, long ChatId)
        {
            ...
            object t = Type.Missing; //значение по умолчанию
            ...
            Document Документ = WORD.Documents.Add(t, t, t, t);
            //работа с текстом
            ...
            using (System.IO.FileStream stream = System.IO.File.OpenRead(FileName))
            {
                ...
                {
                    FileName = $"Список научных работ {NameUser}.pdf"
                };
                Telegram.Bot.Types.Message r = _client.SendDocumentAsync(ChatId, inputOnlineFile, caption: $"Список научных работ пользователя:\n<b>{NameUser} по состоянию на {DateTime.Now.ToLongDateString()}</b>", parseMode: Telegram.Bot.Types.Enums.ParseMode.Html, replyMarkup: null).Result;
            }
